...
# the schema. Should be equal to ALBERT's hidden_size.
...
class Dstc8DataProcessor(object):
  ...
  def __init__(self,
    ...
    self._tokenizer = tokenization.FullTokenizer(
        vocab_file=vocab_file, spm_model_file=spm_model_file, do_lower_case=do_lower_case)
  ...
  def _create_examples_from_turn(self, turn_id, system_utterance,
    ...
    for service, user_frame in user_frames.items():
      ...
      # The input tokens to albert are in the format [CLS] [S1] [S2] ... [SEP]
  ...
  def _tokenize(self, utterance):
    """Tokenize the utterance using word-piece tokenization used by ALBERT.

    Args:
      utterance: A string containing the utterance to be tokenized.

    Returns:
      albert_tokens: A list of tokens obtained by word-piece tokenization of the
        utterance.
      alignments: A dict mapping indices of characters corresponding to start
        ...
        albert_tokens list.
      inverse_alignments: A list of size equal to albert_tokens. Each element is a
    ...
    albert_tokens_start_chars = []
    albert_tokens_end_chars = []
    for token in tokens:
      if token.strip():
        ...
        alignments[char_index] = len(albert_tokens)
        albert_tokens_start_chars.extend([char_index] * len(subwords))
        albert_tokens.extend(subwords)
        # The inclusive ending character index corresponding to the word.
        inclusive_char_end = char_index + len(token) - 1
        alignments[inclusive_char_end] = len(albert_tokens) - 1
        albert_tokens_end_chars.extend([inclusive_char_end] * len(subwords))
      char_index += len(token)
    inverse_alignments = list(
        zip(albert_tokens_start_chars, albert_tokens_end_chars))
    return albert_tokens, alignments, inverse_alignments
...
class InputExample(object):
  ...
  def __init__(self,
    ...
    # The id of each subword in the vocabulary for ALBERT.
  ...
  def add_utterance_features(self, system_tokens, system_inv_alignments,
                             user_tokens, user_inv_alignments):
    """Add utterance related features input to albert.

    Note: this method modifies the system tokens and user_tokens in place to
    make their total length <= the maximum input length for ALBERT model.
    ...
    Args:
      ...
      system_inv_alignments: a list of tuples which denotes the start and end
        charater of the tpken that a albert token originates from in the original
        system utterance.
      ...
      user_inv_alignments: a list of tuples which denotes the start and end
        charater of the token that a albert token originates from in the original
        user utterance.
    """
    # Make user-system utterance input (in ALBERT format)
    # Input sequence length for utterance ALBERT encoder
    ...
    # input to ALBERT, using the tokens for system utterance (sequence A) and
    ...
    # Zero-pad up to the ALBERT input sequence length.
