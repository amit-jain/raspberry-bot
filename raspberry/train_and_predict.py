...
import schema
from raspberry import data_utils
from raspberry import extract_schema_embedding
from raspberry import pred_utils
from raspberry.albert import modeling
from raspberry.albert import optimization
from raspberry.albert import tokenization
...
flags.DEFINE_string("model_ckpt_dir", None,
                    "Directory containing pre-trained BERT checkpoint.")

flags.DEFINE_string("model_prefix", "model.ckpt", "Model prefix to load.")                    
flags.DEFINE_string("model_initial_qualifier", "best", "Model pre-trained qualifier to be added to prefix to load.")                    
...
flags.DEFINE_string(
    ...
    "The output base directory for all outputs.")

flags.DEFINE_string(
    "checkpoint_dir", "checkpoint",
    "The path under output base directory where checkpoints are written.")

flags.DEFINE_string(
    "prediction_dir", "predictions",
    "The path under output base directory for all prediction outputs.")

flags.DEFINE_string(
    "schema_embedding_dir", "schema",
    "Path under base output .npy file for embedding of entities (slots, values,"
    " intents) in the dataset_split's schema are stored.")

flags.DEFINE_string(
    "dialogues_example_dir", "examples",
...
class SchemaGuidedDST(object):
  ...
  def __init__(self, model_config, use_one_hot_embeddings):
...
def _model_fn_builder(model_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    schema_guided_dst = SchemaGuidedDST(model_config, use_one_hot_embeddings)
...
def _create_schema_embeddings(model_config, schema_embedding_file, model_init_ckpt, 
  ...
  if not tf.io.gfile.exists(schema_embedding_dir):
    tf.io.gfile.makedirs(schema_embedding_dir)
  is_per_host = tf.estimator.tpu.InputPipelineConfig.PER_HOST_V2
  schema_emb_run_config = tf.estimator.tpu.RunConfig(
      master=FLAGS.master,
  ...
  # Prepare model for embedding a natural language descriptions.
  schema_emb_model_fn = extract_schema_embedding.model_fn_builder(
      model_config=model_config,
      init_checkpoint=model_init_ckpt,
      use_tpu=FLAGS.use_tpu,
      use_one_hot_embeddings=FLAGS.use_one_hot_embeddings)
  # If TPU is not available, this will fall back to normal Estimator on CPU
  # or GPU.
  schema_emb_estimator = tf.estimator.tpu.TPUEstimator(
      use_tpu=FLAGS.use_tpu,
...
def main(_):
  ...
  # Load default pre-trained model or load last checkpoint if available
  model_init_ckpt = os.path.join(FLAGS.model_ckpt_dir, 
    FLAGS.model_prefix + "-" + FLAGS.model_initial_qualifier)
  if os.path.exists(os.path.join(ckpt_dir, FLAGS.checkpoint_dir)):
      model_init_ckpt = os.path.join(FLAGS.output_base_dir, FLAGS.checkpoint_dir)

  model_config_file = os.path.join(FLAGS.model_ckpt_dir, "albert_config.json")
  ...
  dial_file_name = "{}_{}_examples.tf_record".format(task_name,
                                                     FLAGS.dataset_split)
  tf.io.gfile.makedirs(os.path.join(FLAGS.output_base_dir, FLAGS.dialogues_example_dir))
  dial_file = os.path.join(FLAGS.output_base_dir, FLAGS.dialogues_example_dir, dial_file_name)
  ...
  model_config = modeling.AlbertConfig.from_json_file(model_config_file)
    ...
    for ckpt_num in ckpt_nums:
      ...
      predictions = estimator.predict(
          ...
          checkpoint_path=os.path.join(ckpt_dir,
                                       FLAGS.model_prefix + "-%s" % ckpt_num))

      # Write predictions to file in DSTC8 format.
      dataset_mark = os.path.basename(FLAGS.dstc8_data_dir)
      prediction_base_dir = os.path.join(FLAGS.output_base_dir, FLAGS.prediction_dir)
      prediction_dir = os.path.join(
          prediction_base_dir, "pred_res_{}_{}_{}_{}".format(
...
if __name__ == "__main__":
  ...
  flags.mark_flag_as_required("model_ckpt_dir")
  flags.mark_flag_as_required("dataset_split")
  flags.mark_flag_as_required("task_name")
  flags.mark_flag_as_required("output_base_dir")
